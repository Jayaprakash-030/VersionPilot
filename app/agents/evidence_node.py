...
def evidence_node(state: VersionPilotState) -> dict:
    ...
    migration_failed_steps: list[str] = list(
        state.get("migration_analysis_failed_steps", [])
    )
    ...
    record_migration_check(
        "fetch_dependency_names", dep_names_result.get("status", "ok")
    )
    dependencies: list[dict] = dep_names_result.get("dependencies", [])
    ...
    for dep in dependencies:
        ...
        notes_result = registry.fetch_dependency_release_notes(name, version)
        provenance.append(
            {
                "source": f"release_notes:{name}",
                "timestamp": _now_iso(),
                "status": notes_result.get("status", "ok"),
            }
        )
        record_migration_check(
            f"release_notes:{name}", notes_result.get("status", "ok")
        )
        ...
        pkg_rules = extractor.build_rules_dict(name, notes_text)
        extraction_status = extractor.last_extraction_status
        if extraction_status not in {"ok", "unavailable", "error", "skipped"}:
            extraction_status = "ok"
        provenance.append(
            {
                "source": f"rules_extraction:{name}",
                "timestamp": _now_iso(),
                "status": extraction_status,
            }
        )
        record_migration_check(f"rules_extraction:{name}", extraction_status)
        if pkg_rules:
        ...
        record_migration_check(
            f"changelog:{name}", changelog_result.get("status", "ok")
        )
    ...
    if repo_path and "deprecated_api_scan" not in skip_steps:
        try:
            ...
            record_migration_check(
                "deprecated_api_scan", scan_result.get("status", "ok")
            )
    ...
    # ------------------------------------------------------------------
    migration_result = registry.generate_migration_plan(
        deprecated_findings, breaking_change_analysis
    )
    provenance.append(
        {
